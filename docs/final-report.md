...
![系统总体架构图](screenshots/17-system-architecture.png)
...
![数据库表结构截图](screenshots/15-db-schema.png)
...
![新增资源与写入用量表单](screenshots/04-resource-usage-forms.png)

![资源清单、查询和状态操作](screenshots/05-resource-table.png)
...
![系统总览页面](screenshots/01-dashboard-overview.png)
...
![成本分析与优化模拟页面](screenshots/02-cost-analysis.png)
...
![告警中心、CSV 导入和运维周报入口](screenshots/03-operations-alerts.png)
...
![华为云开发者空间项目目录截图](screenshots/09-devspace-directory.png)
...
![openGauss 镜像拉取成功截图](screenshots/10-opengauss-image-pull.png)
...
![Docker Compose 服务运行状态截图](screenshots/11-compose-services.png)

连接并验证 openGauss。该镜像中 `gsql` 不在默认 `PATH`，需要手动补充。如需进入容器排查，可先执行 `docker exec -it cloudcostlab-opengauss bash`，再手动设置 `LD_LIBRARY_PATH` 后调用 `/usr/local/opengauss/bin/gsql`。

![openGauss 数据查询截图](screenshots/14-opengauss-query.png)
...
![FastAPI Swagger 接口文档](screenshots/07-swagger-api-docs.png)
...
![CloudCostLab 前端完整页面](screenshots/01-dashboard-fullpage.png)
...
![Spark 批处理任务运行截图](screenshots/12-spark-run.png)
...
![Spark 聚合结果写入 openGauss 截图](screenshots/13-spark-snapshot-query.png)

## 4 测试评估

### 4.1 测试方法

本项目主要从接口、页面、数据库和容器部署四个方面进行验证。接口层面通过 FastAPI 的 Swagger 文档和后端测试用例验证资源、用量、分析、告警和导出接口；页面层面通过浏览器访问华为云开发者空间中暴露的前端地址，检查各个按钮和表单是否能正常触发后端逻辑；数据库层面通过 openGauss 查询确认资源、用量、分析快照、告警和审计日志均已落库；部署层面通过 Docker Compose 状态和访问地址确认前端、后端、数据库容器运行正常。

本地验证时，后端测试用例通过，前端生产构建也能完成。部署验证时，前端页面、后端 Swagger 文档和 `/api/health` 接口均能正常访问。

![scripts/cloud-preflight.sh 自检脚本测试通过](screenshots/16-preflight-check.png)


### 4.2 功能测试结果

功能测试覆盖了课程要求中的主要流程，包括资源管理、用量写入、数据库读写、前后端交互、容器化部署和 Spark 批处理。测试结果汇总如下。

| 测试类别 | 验证内容 | 结果 |
| --- | --- | --- |
| 基础访问 | 前端首页、后端 Swagger、`/api/health` | 通过 |
| 资源管理 | 新增、查询、状态切换、删除资源 | 通过 |
| 用量采集 | 写入 CPU、内存、存储、网络和利用率数据，自动计算成本和碳排放 | 通过 |
...
| Spark 批处理 | Spark 通过 JDBC 读取 openGauss 明细并写回聚合快照 | 通过 |

### 4.3 数据库读写验证

数据库验证重点是确认前端操作确实会通过后端写入 openGauss，而不是只停留在页面展示。测试时新增资源、写入用量、执行分析、生成告警后，openGauss 中对应表均能查询到数据变化。其中：

- `cloud_resources` 能看到资源记录和状态变化。
- `usage_records` 能看到用量记录、估算成本和碳排放。
- `analytics_snapshots` 能看到 API 和 Spark 写入的聚合快照。
- `cloud_alerts` 能看到告警规则执行结果及确认状态。
- `audit_events` 能看到资源更新、用量写入、分析、告警等审计事件。

![最近用量、预算风险和审计日志](screenshots/06-audit-usage-risk.png)

### 4.4 部署验证

部署后在华为云开发者空间中完成访问验证：前端通过 `http://113.44.208.1:8080` 访问，后端通过 `http://113.44.208.1:8000/docs` 查看接口文档，健康检查接口返回 `ok`。Docker Compose 中前端、后端和 openGauss 三个核心容器保持运行状态。openGauss 查询使用完整 `LD_LIBRARY_PATH + /usr/local/opengauss/bin/gsql` 命令执行，可以查询到资源、用量、分析快照和告警数据。
